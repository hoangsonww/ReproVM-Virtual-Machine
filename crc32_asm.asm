; Standalone CRC32 calculator using Linux x86-64 syscalls.
; Usage: ./crc32_asm <file>
; Outputs: 8 hex digits (lowercase) + newline

global _start

section .data
usage_msg      db "Usage: crc32_asm <file>", 10
usage_len      equ $ - usage_msg

err_open      db "Error: cannot open file", 10
err_open_len  equ $ - err_open

err_read      db "Error: read failure", 10
err_read_len  equ $ - err_read

hex_digits db "0123456789abcdef"

; CRC32 table for polynomial 0xEDB88320 (standard)
crc32_table:
...
_start:
    ...
    mov rsi, [rsp+16]       ; filename (argv[1])

    ; open file: syscall open(filename, O_RDONLY=0)
    xor rdx, rdx            ; flags = 0
    mov rax, 2              ; sys_open
...
.process_bytes:
    movzx r8d, byte [rel buffer + rsi]  ; current byte
    mov eax, ebx
    xor eax, r8d
    and eax, 0xFF                  ; table index
    mov edx, ebx
    shr edx, 8                     ; crc >> 8
    mov r9d, eax
    mov eax, dword [rel crc32_table + r9*4]
    xor edx, eax
    mov ebx, edx                  ; new crc
    inc rsi
    cmp rsi, rcx
...
.finalize:
    xor ebx, 0xFFFFFFFF           ; finalize crc

    ; convert to 8 hex digits (most significant nibble first)
    mov eax, ebx                  ; working copy
    xor rcx, rcx                  ; counter = 0
    lea rdi, [rel outbuf]         ; output buffer pointer

.hex_loop:
    mov rdx, eax
    shr rdx, 28                   ; top nibble
    and rdx, 0xF
    cmp rdx, 9
    jle .digit
    add rdx, 'a' - 10
...
.store:
    mov byte [rdi + rcx], dl
    shl eax, 4                    ; move next nibble into top
    inc rcx
    cmp rcx, 8
    jne .hex_loop
